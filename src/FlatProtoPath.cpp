...
namespace proto
{
    ...
    std::size_t FlatProtoPath::compute_size(const std::vector<PathStep>& steps) noexcept
    {
        ...
        for (const auto& s : steps) total += s.field_name.size();
        return total;
    }

    void FlatProtoPath::encode_into(const std::vector<PathStep>& steps,
                                    std::byte* dst, std::size_t dst_size)
    {
        ...
        for (uint32_t i = 0; i < step_count; ++i)
        {
            ...
            write_bytes(dst + steps_offset + i * sizeof(StepRecord),
                        &rec, sizeof(rec));

            if (rec.name_len > 0)
                write_bytes(dst + cur_name_off,
                            src.field_name.data(), rec.name_len);

            cur_name_off += rec.name_len;
        }
    }

    FlatProtoPath::FlatProtoPath(std::span<const std::byte> bytes)
            : owned_(), view_(bytes)
    {
        validate();
    }

    FlatProtoPath::FlatProtoPath(const std::vector<PathStep>& steps)
            : owned_(compute_size(steps)), view_()
    {
    ...
    FlatPathStep FlatProtoPath::operator[](std::size_t i) const
    {
        ...
        const StepRecord* recs = reinterpret_cast<const StepRecord*>(
                view_.data() + sizeof(Header));
    ...
    void FlatProtoPath::validate() const
    {
        ...
        const std::size_t steps_end =
                sizeof(Header) + static_cast<std::size_t>(h.step_count) * sizeof(StepRecord);
