...
namespace proto
{
                           ...
                           const std::string& root_message_name)
    {
        ...
        while (!q.empty())
        {
            const ProtoSchemeMap::MessageDef* m = q.front(); q.pop();
            for (const auto& f : m->fields_in_order)
            {
                if (f.kind != FieldKind::Message) continue;
    ...
    std::size_t ProtoSchemeFlatMap::compute_size(const ProtoSchemeMap& built_map,
                                                 const std::string& root_message_name)
    {
    ...
    void ProtoSchemeFlatMap::encode_into(const ProtoSchemeMap& built_map,
                                         const std::string& root_message_name,
                                         std::byte* dst, std::size_t dst_size)
    {
        ...
        for (uint32_t mi = 0; mi < message_count; ++mi)
        {
            ...
            mrec.reserved     = 0;
            write_bytes(dst + messages_offset + mi * sizeof(MessageRecord),
                        &mrec, sizeof(mrec));
            ...
            for (uint32_t fi = 0; fi < mrec.fields_count; ++fi)
            {
                ...
                write_bytes(dst + fields_offset + (cur_field_idx + fi) * sizeof(FieldRecord),
                            &frec, sizeof(frec));
    ...
    ProtoSchemeFlatMap::ProtoSchemeFlatMap(const std::string& proto_scheme_text,
                                           const std::string& root_message_name)
            : owned_(), view_()
    {
    ...
    {
        if (view_.size() < sizeof(Header)) return 0;
    ...
    const ProtoSchemeFlatMap::MessageRecord&
    ProtoSchemeFlatMap::message_at(std::size_t idx) const
    {
        const Header& h = header();
        if (idx >= h.message_count)
            throw std::out_of_range("ProtoSchemeFlatMap::message_at: out of range");
        const auto* recs = reinterpret_cast<const MessageRecord*>(
                view_.data() + h.messages_offset);
        return recs[idx];
    }

    const ProtoSchemeFlatMap::FieldRecord*
    ProtoSchemeFlatMap::field_ptr(std::size_t idx) const
    {
        const Header& h = header();
        if (idx >= h.field_count) return nullptr;
        const auto* recs = reinterpret_cast<const FieldRecord*>(
                view_.data() + h.fields_offset);
        return recs + idx;
    }

    const ProtoSchemeFlatMap::FieldRecord*
    ProtoSchemeFlatMap::find_field(const MessageRecord& msg, int field_number) const
    {
        const Header& h = header();
        const auto* base = reinterpret_cast<const FieldRecord*>(
                view_.data() + h.fields_offset);
